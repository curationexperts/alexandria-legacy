...
describe Exporter::LocalAuthorityExporter do
  ...
  describe '#run' do
    ...
    let(:headers) { ['id', 'public_uri', 'name', 'name', 'name', 'name'] }
    ...
    it 'exports the local authorities' do
      ...
      expect(contents[1].split(',')).to eq [agent.id, agent.public_uri, agent.foaf_name]
      ...
      person = line2[0] == justin.id ? justin : alicia
      expect(line2).to eq [person.id, person.public_uri, person.foaf_name]

      person = line3[0] == justin.id ? justin : alicia
      expect(line3).to eq [person.id, person.public_uri, person.foaf_name]

      expect(contents[4].split(',')).to eq [devs.id, devs.public_uri, devs.foaf_name]
      expect(contents[5].split(',')).to eq [dce.id, dce.public_uri, dce.foaf_name]

      line6 = contents[6].split(',')
      line7 = contents[7].split(',')

      # We don't know what order the topics will be in.
      topic = line6[0] == fun.id ? fun : tools
      expect(line6).to eq [topic.id, topic.public_uri] + topic.label

      topic = line7[0] == fun.id ? fun : tools
      expect(line7).to eq [topic.id, topic.public_uri] + topic.label
    end
  end  # run
